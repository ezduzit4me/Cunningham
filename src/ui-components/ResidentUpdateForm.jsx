...
import { updateResident } from "../graphql/mutations";
const client = generateClient();
export default function ResidentUpdateForm(props) {
  const {
    id: idProp,
    resident: residentModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = residentRecord
      ? { ...initialValues, ...residentRecord }
      : initialValues;
    setName(cleanValues.name);
    setApartment_no(cleanValues.apartment_no);
    setMove_in_date(cleanValues.move_in_date);
    setMobile_phone(cleanValues.mobile_phone);
    setParking_no(cleanValues.parking_no);
    setVehicle_rego(cleanValues.vehicle_rego);
    setPet_type(cleanValues.pet_type);
    setPet_weight(cleanValues.pet_weight);
    setStorage_no(cleanValues.storage_no);
    setRe_name(cleanValues.re_name);
    setRe_mobile(cleanValues.re_mobile);
    setRe_email(cleanValues.re_email);
    setEmail(cleanValues.email);
    setErrors({});
  };
  const [residentRecord, setResidentRecord] = React.useState(residentModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getResident.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getResident
        : residentModelProp;
      setResidentRecord(record);
    };
    queryData();
  }, [idProp, residentModelProp]);
  React.useEffect(resetStateValues, [residentRecord]);
  const validations = {
    name: [],
    apartment_no: [],
    move_in_date: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          parking_no: parking_no ?? null,
          vehicle_rego: vehicle_rego ?? null,
          pet_type: pet_type ?? null,
          pet_weight: pet_weight ?? null,
          storage_no: storage_no ?? null,
          re_name: re_name ?? null,
          re_mobile: re_mobile ?? null,
          re_email: re_email ?? null,
          email: email ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateResident.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "ResidentUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || residentModelProp)}
